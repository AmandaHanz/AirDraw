...
def draw(shape, x1, x2, y1, y2, color, thickness):
    if shape == "line":
        ...
        cv2.circle(img, ((x2 + x1) // 2, (y2 + y1) // 2), int((np.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)) / 2), color,
                   thickness)
...
def nav_bar(nav):
    if nav:
        ...
        cv2.putText(nav_img, 'text', (5 * division, 29 + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, dct['color'],
                    1 * int(shapes_active[3] / 2), cv2.LINE_AA)
        cv2.putText(nav_img, 'erase', (6 * division + 5, 29 + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, dct['color'],
                    1 * int(shapes_active[4] / 2), cv2.LINE_AA)
        cv2.putText(nav_img, 'marker', (8 * division, 29 + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, dct['color'],
                    1 * int(shapes_active[5] / 2), cv2.LINE_AA)
        cv2.putText(nav_img, 'grid', (10 * division, 29 + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, dct['color'], 1,
                    cv2.LINE_AA)

        cv2.putText(nav_img, f"thickness :{dct['thickness']}", (11 * division, 29 + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    dct['color'], 1, cv2.LINE_AA)
...
def track(event, x, y):
    ...
    if text:
        img = prev_img.copy()
        cv2.putText(img, ''.join(lst), (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, dct['color'], dct['thickness'],
                    cv2.LINE_AA)
        if event == 1:
            cv2.putText(img, ''.join(lst), (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, dct['color'], dct['thickness'],
                        cv2.LINE_AA)
            ...
            if ((c == x and d == y) or tab) and len(points) > 10:
                if count >= 5 or tab:
                    ...
                    if is_line(np.array(points)):
                        ...
                        if is_c[0]:
                            img = prev_img.copy()
                            draw(shape="circle", x1=x1, x2=is_c[1][0], y1=y1, y2=is_c[1][1], color=dct['color'],
...
img_count = 1
colors = {1: ["red", (0, 0, 255), 15], 2: ['green', (0, 255, 0), 10], 3: ['blue', (255, 0, 0), 10],
          4: ["white", (255, 255, 255), 10], 5: ["yellow", (0, 225, 255), 10], 6: ["orange", (0, 165, 255), 10]}
